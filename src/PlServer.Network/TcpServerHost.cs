        ...
        try
        {
            await using (connection.ConfigureAwait(false))
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    if (bytesRead > _options.MaxFrameSize)
                    {
                        continue;
                    }

                    var frame = buffer.AsSpan(0, bytesRead).ToArray();
                    await _receivePipeline
                        .ReceiveAsync(connection.Context, frame, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
        }
